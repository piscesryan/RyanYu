...
public class Uploader {
	...
	// 保存路径---------------------------------更改了---------------------------------------
	private String savePath = "filess/files/";
	// 文件允许格式
	private String[] allowFiles = { ".rar", ".doc", ".docx", ".xls", ".zip", ".pdf",
...
//System.out.println("113-savePath"+savePath);
			if (!this.checkFileType(this.originalName)) {
				this.state = this.errorInfo.get("TYPE");
				return;
			}
try{
			this.fileName = this.getName(this.originalName);}catch(Throwable e){e.printStackTrace();}
	 ...
	 * @param fieldName
	 */
	public void uploadBase64(String fieldName)  throws Throwable{
		String savePath = this.getFolder(this.savePath);
	...
	private String getName(String fileName) throws Throwable{
		//Random random = new Random();
fileName = new String(fileName.getBytes(),"UTF-8");//System.out.println("257-fileName"+fileName);		
		return fileName;
	}
	/**
...
//System.out.println("270-path"+path);		
//System.out.println("270-this.getPhysicalPath(path)"+this.getPhysicalPath(path));			
		if (!dir.exists()) {
			try {
				dir.mkdirs();
			} catch (Exception e) {
	...
	private String getPhysicalPath(String path) {
		...
		String realPath = this.request.getSession().getServletContext()
				.getRealPath("/");
//System.out.println("299-realPath"+realPath);
//System.out.println("299-getPhysicalPath"+new File(realPath) + "/" + path);
...
//System.out.println("savePath-342:"+savePath);		
	}
