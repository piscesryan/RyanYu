if (!PrimeFaces.isIE(8)) {
	...
	/*
	 * ! XRegExp v2.0.0 (c) 2007-2012 Steven Levithan <http://xregexp.com/> MIT
	 * License
	 */
	var XRegExp;
	XRegExp = XRegExp
			|| (function(c) {
				...
				}, e = {}, o = {}, n = [], b = "default", l = "class", A = {
					"default" : /^(?:\\(?:0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|[1-9]\d*|x[\dA-Fa-f]{2}|u[\dA-Fa-f]{4}|c[A-Za-z]|[\s\S])|\(\?[:=!]|[?*+]\?|{\d+(?:,\d*)?}\??)/,
					"class" : /^(?:\\(?:[0-3][0-7]{0,2}|[4-7][0-7]?|x[\dA-Fa-f]{2}|u[\dA-Fa-f]{4}|c[A-Za-z]|[\s\S]))/
				}, z = /\$(?:{([\w$]+)}|(\d\d?|[\s\S]))/g, x = /([\s\S])(?=[\s\S]*\1)/g, r = /^(?:[?*+]|{\d+(?:,\d*)?})\??/, B = t.exec
						.call(/()??/, "")[1] === c, j = RegExp.prototype.sticky !== c, g = false, y = "gim"
						+ (j ? "y" : "");
				function v(E, C, D) {
					var F;
				...
				function k(C) {
					return (C.global ? "g" : "") + (C.ignoreCase ? "i" : "")
							+ (C.multiline ? "m" : "")
							+ (C.extended ? "x" : "") + (C.sticky ? "y" : "")
				}
				function s(F, D, E) {
					if (!q.isRegExp(F)) {
					...
					if (F.xregexp && !F.xregexp.isNative) {
						F = v(q(F.source, C),
								F.xregexp.captureNames ? F.xregexp.captureNames
										.slice(0) : null)
					} else {
				...
				function f(D, C) {
					return Object.prototype.toString.call(D).toLowerCase() === "[object "
							+ C + "]"
				}
				function m(C) {
					C = C || {};
				...
				function d(G, H, I, C) {
					...
					try {
						while (E--) {
							J = n[E];
							if ((J.scope === "all" || J.scope === I)
									&& (!J.trigger || J.trigger.call(C))) {
								J.pattern.lastIndex = H;
								F = e.exec.call(J.pattern, G);
								if (F && F.index === H) {
									K = {
										output : J.handler.call(C, F, I),
										match : F
									};
									break
								}
				...
				q = function(I, D) {
					...
					q.forEach(D, /[\s\S]/, function(L) {
						if (y.indexOf(L[0]) < 0) {
							throw new SyntaxError(
									"invalid regular expression flag " + L[0])
						}
					});
					while (J < I.length) {
						F = d(I, J, K, E);
					...
					return v(new RegExp(C.join(""), t.replace.call(D,
							/[^gimy]+/g, "")),
							E.hasNamedCapture ? E.captureNames : null)
				};
				i = {
					on : function(E, D, C) {
						C = C || {};
				...
				q.matchChain = function(D, C) {
					return (function E(F, K) {
						...
						return ((K === C.length - 1) || !J.length) ? J : E(J,
								K + 1)
					}([ D ], 0))
				};
				...
				q.union = function(C, E) {
					var F = /(\()(?!\?)|\\([1-9]\d*)|\\[\s\S]|\[(?:[^\\\]]|\\[\s\S])*]/g, H = 0, J, K, L = function(
							N, O, P) {
						var M = K[H - J];
						if (O) {
							++H;
				...
				e.exec = function(H) {
					...
					if (F) {
						if (!B && F.length > 1 && w(F, "") > -1) {
							D = new RegExp(this.source, t.replace.call(k(this),
									"g", ""));
							t.replace
									.call(
											String(H).slice(F.index),
											D,
											function() {
												var I;
				...
				e.replace = function(F, G) {
					...
					if (f(G, "function")) {
						...
						D = t.replace.call(I, F, function() {
							var J = arguments;
							return t.replace.call(String(G), z, function(L, K,
									N) {
								var M;
								if (K) {
									M = +K;
				...
				p(/(?:\(\?#[^)]*\))+/,
						function(C) {
							return t.test.call(r, C.input.slice(C.index
									+ C[0].length)) ? "" : "(?:)"
						});
				p(
						/\\k<([\w$]+)>/,
						function(D) {
							var C = isNaN(D[1]) ? (w(this.captureNames, D[1]) + 1)
									: +D[1], E = D.index + D[0].length;
				...
				p(/\(\?P?<([\w$]+)>/, function(C) {
					if (!isNaN(C[1])) {
						throw new SyntaxError(
								"can't use integer as capture name " + C[0])
					}
				...
				p(
						/\\(\d+)/,
						function(C, D) {
							if (!(D === b && /^[1-9]/.test(C[1]) && +C[1] <= this.captureNames.length)
									&& C[1] !== "0") {
								throw new SyntaxError(
		...
		var SyntaxHighlighter = function() {
			var l = {
				...
				regexLib : {
					...
					multiLineDoubleQuotedString : XRegExp(
							'"([^\\\\"]|\\\\.)*"', "gs"),
					multiLineSingleQuotedString : XRegExp(
							"'([^\\\\']|\\\\.)*'", "gs"),
				...
				toolbar : {
					getHtml : function(N) {
						...
						function Q(T, S) {
							return l.toolbar.getButtonHtml(T, S,
									l.config.strings[S])
						}
					...
					getButtonHtml : function(M, N, L) {
						N = v(N);
						return '<span><a href="#" class="toolbar_item command_'
								+ N + " " + N + '">' + v(L) + "</a></span>"
					},
					handler : function(Q) {
					...
					items : {
						...
						expandSource : {
							getHtml : function(L) {
								...
								return l.toolbar
										.getButtonHtml(L, "expandSource", M ? M
												: l.config.strings.expandSource)
							},
							execute : function(L) {
								var M = J(L.id);
								j(M, "collapsed")
						...
						help : {
							execute : function(L) {
								var M = x("", "_blank", 500, 250,
										"scrollbars=0"), N = M.document;
				...
				findElements : function(Q, P) {
					var S = P ? [ P ] : r(document
							.getElementsByTagName(l.config.tagName)), N = l.config, L = [];
					if (N.useScriptTags) {
						S = S.concat(K())
					}
					if (S.length === 0) {
				...
				highlight : function(R, P) {
					...
					for ( var Q = 0, N = L.length; Q < N; Q++) {
						...
						if (O["html-script"] == "true"
								|| l.defaults["html-script"] == true) {
			...
			function v(L) {
				return document.createElement("div").appendChild(
						document.createTextNode(L)).parentNode.innerHTML
						.replace(/"/g, "&quot;")
			}
			function E(M, L) {
			...
			function h(P, T, Q) {
				...
				var L = Q != true ? P.childNodes : [ P.parentNode ], R = {
					"#" : "id",
					"." : "className"
				}[T.substr(0, 1)] || "nodeName", M, S;
			...
			function o(R) {
				var N, M = {}, O = XRegExp("^\\[(?<values>(.*?))\\]$"), S = 0, P = XRegExp(
			...
			function u(P, Q) {
				...
				function R(S, U, T) {
					return S.substr(0, U) + M.substr(0, T)
							+ S.substr(U + 1, S.length)
				}
				P = I(P, function(S) {
					if (S.indexOf(O) == -1) {
						return S
			...
			function e(Q) {
				var R = Q.target, P = C(R, ".syntaxhighlighter"), L = C(R,
						".container"), T = document.createElement("textarea"), S;
				if (!L || !P || h(L, "textarea")) {
					return
				}
				S = s(P.id);
			...
			l.HtmlScript = function(Q) {
				var U = g(Q), T, L = new l.brushes.Xml(), S = null, N = this, P = "getDiv getHtml init"
						.split(" ");
				if (U == null) {
					return
				}
				...
				function M(ae, X) {
					var W = ae.code, ad = [], ac = T.regexList, aa = ae.index
							+ ae.left.length, af = T.htmlScript, ag;
			...
			l.Highlighter.prototype = {
				...
				getLineHtml : function(O, L, N) {
					var M = [ "line", "number" + L, "index" + O,
							"alt" + (L % 2 == 0 ? 1 : 2).toString() ];
				...
				getLineNumbersHtml : function(R, M) {
					var P = "", Q = w(R).length, N = parseInt(this
							.getParam("first-line")), S = this
					...
					for ( var O = 0; O < Q; O++) {
						var L = M ? M[O] : N + O, R = L == 0 ? l.config.space
								: c(L, S);
				...
				getCodeLinesHtml : function(P, T) {
					P = a(P);
					var W = w(P), Q = this.getParam("pad-line-numbers"), S = parseInt(this
							.getParam("first-line")), P = "", U = this
							.getParam("brush");
					for ( var O = 0, N = W.length; O < N; O++) {
						var V = W[O], L = /^(&nbsp;|\s)+/.exec(V), R = null, M = T ? T[O]
								: S + O;
						if (L != null) {
							R = L[0].toString();
							V = V.substr(R.length);
						...
						P += this.getLineHtml(O, M,
								(R != null ? '<code class="' + U + ' spaces">'
										+ R + "</code>" : "")
										+ V)
					}
					return P
				...
				getMatchesHtml : function(L, R) {
					...
					for ( var P = 0, N = R.length; P < N; P++) {
						...
						M = O(Q);
						U += y(L.substr(S, Q.index - S), M + "plain")
								+ y(Q.value, M + Q.css);
				...
				getHtml : function(O) {
					...
					N = '<div id="'
							...
							+ v(M.join(" "))
							+ '">'
							+ (this.getParam("toolbar") ? l.toolbar
									.getHtml(this) : "")
							+ '<table border="0" cellpadding="0" cellspacing="0">'
							+ this.getTitleHtml(this.getParam("title"))
							+ "<tbody><tr>"
							+ (gutter ? '<td class="gutter">'
									+ this.getLineNumbersHtml(O) + "</td>" : "")
							+ '<td class="code"><div class="container">' + N
				...
				forHtmlScript : function(L) {
					...
					this.htmlScript = {
						...
						code : XRegExp("(?<left>" + L.left.source
								+ ")(?<code>.*?)(?<right>" + M.end + ")", "sgi")
					}
				}
			};
			return l
	...
	(function() {
		...
		function a() {
			function b(g, c) {
				var e = SyntaxHighlighter.Match, d = g[0], j = XRegExp.exec(d,
						XRegExp("(&lt;|<)[\\s\\/\\?!]*(?<name>[:\\w-\\.]+)",
								"xg")), k = [];
				if (g.attributes != null) {
					...
					while ((f = XRegExp.exec(d, i, h)) != null) {
						...
						k.push(new e(f.value, g.index + f.index
								+ f[0].indexOf(f.value), "string"));
						h = f.index + f[0].length
					}
				}
				if (j != null) {
					k.push(new e(j.name, g.index + j[0].indexOf(j.name),
							"keyword"))
				}
				return k
			}
